...
사용자는 `ssh <vm슬러그>@ssh.pickle.pnuops.com` 한 줄로 자기 VM에 접속하고, 게이트웨이가
...
- **차단 반영**: 비밀번호 접속은 기본적으로 거부하고, 관리자가 내린 전역 차단과
  VM별 차단을 매 연결에서 확인합니다.
...
Go 1.26이 필요합니다. 단독으로 돌려볼 수 있는 서비스는 아니며, pickle-api의 내부 API와
WireGuard 터널이 있는 환경을 전제합니다. `scripts/systemd/`의 유닛 3종이 서비스 정의의
단일 출처이므로, 유닛만 고치고 호스트에 동기화하지 않으면 실행 중인 서비스는 그대로
남습니다.
